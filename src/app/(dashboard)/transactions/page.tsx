...
export default function TransactionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      const result = await supabase
        .from("transactions")
        .insert({ ...txnData, user_id: user.id });
      error = result.error;
    }
          ...
          <div className="flex flex-col md:flex-row gap-2 md:items-center justify-between">
            <div className="flex gap-2 overflow-x-auto pb-1 md:pb-0 hide-scrollbar">
              ...
              <select
                value={filterDay ?? ""}
                onChange={(e) =>
                  setFilterDay(e.target.value ? parseInt(e.target.value) : null)
                }
                className="input w-auto text-sm"
              >
              ...
              <div className="flex p-1 bg-bg-secondary rounded-lg border border-border/50 shrink-0">
                {(["all", "income", "expense"] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${filter === f ? "bg-bg-card text-text-primary shadow-sm" : "text-text-secondary hover:text-text-primary"}`}
                  >
                    {f === "all" ? "All" : f === "income" ? "💰" : "💸"}
                  </button>
                ))}
                  ...
                  <div key={date}>
                    ...
                    <div className="space-y-2">
                      {txns.map((txn) => (
                        <div
                          key={txn.id}
                          className="card p-3 flex items-center justify-between group hover:border-border/80 transition-all cursor-pointer"
                          onClick={() => openEditModal(txn)}
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className="w-10 h-10 rounded-xl bg-bg-secondary flex items-center justify-center text-lg"
                              style={{
                          ...
                          <div className="flex flex-col items-end gap-0.5">
                            <p
                              className={`font-bold text-[15px] tracking-tight ${txn.type === "income" ? "text-accent-green" : "text-text-primary"}`}
                            >
                              {txn.type === "income" ? "+" : "-"}
          ...
          <div className="flex rounded-lg overflow-hidden border border-border">
            {(["income", "expense"] as const).map((t) => (
              <button
                ...
                className={`flex-1 py-3 font-medium transition-all ${formType === t ? (t === "expense" ? "bg-accent-red text-white" : "bg-accent-green text-white") : "bg-bg-secondary text-text-secondary"}`}
              >
